...
$return .= <<<EOF
,Вам для просмотра скрытого содержания необходимо иметь на балансе: {$creditsrequire}.<br />Ваш текущий баланс: {$_G['member']['credits']}.</div>
EOF;
?><?php 
return $return;
}
...
?><?php
$return = <<<EOF
<div class="locked">Поздравляем! Вам доступно данное содержание для просмотра, так как Вы имеете Золота на балансе: {$creditsrequire} Скрытое содержание смотрите ниже...</div>
{$message}<br /><br />
...
?><?php
$return = <<<EOF
<div class="showhide"><h4>Скрытое содержание</h4>\\1</div>
...
$return .= <<<EOF
, скрытое содержание будет доступно после отправки сообщения! <a href="forum.php?mod=post&amp;action=reply&amp;fid={$_G['fid']}&amp;tid={$_G['tid']}" onclick="showWindow('reply', this.href)">Отправить сообщение!</a>.</center></div>
EOF;
?><?php 
return $return;
}
