...
<div class="pinf">
<?php if($multiple) { ?><strong>Вы можете выбрать несколько вариантов<br />Опросы</strong><?php if($maxchoices) { ?>: ( Максимально <?php echo $maxchoices;?> пунктов )<?php } } else { ?><strong>Выберите ваш вариант<br />Опросы</strong><?php } if($visiblepoll && $_G['group']['allowvote']) { ?> , Просмотр результатов после голосования<?php } ?>, Всего проголосовало: <?php echo $voterscount;?>
<?php if(!$visiblepoll && ($overt || $_G['adminid'] == 1 || $thread['authorid'] == $_G['uid']) && $post['invisible'] == 0) { ?>
<a href="forum.php?mod=misc&amp;action=viewvote&amp;tid=<?php echo $_G['tid'];?>" onclick="showWindow('viewvote', this.href)"><br />Участники опроса</a>
<?php } ?>
</div>
...
Период голосования:
<strong>
<?php if($_G['forum_thread']['remaintime']['0']) { ?><?php echo $_G['forum_thread']['remaintime']['0'];?>  дн. <?php } if($_G['forum_thread']['remaintime']['1']) { ?><?php echo $_G['forum_thread']['remaintime']['1'];?>  час<?php } ?>
<?php echo $_G['forum_thread']['remaintime']['2'];?>  мин
</strong>
</p>
<?php } elseif($expiration && $expirations < TIMESTAMP) { ?>
<p class="ptmr"><strong>Голосование в настоящее время закрыто</strong></p>
<?php } ?>
...
<p class="imgfc">
<span class="z"><?php echo $option['votes'];?>Голосов</span>
...
<button class="pn" type="submit" disabled="disabled" name="pollsubmit" id="pollsubmit" value="true"<?php if($post['invisible'] < 0) { ?> disabled="disabled"<?php } ?>><span>Отправить</span></button>
<?php if($overt) { ?>
(Это публичный опрос. Ваше голосование будут видеть другие пользователи)
<?php } } elseif(!$allwvoteusergroup) { ?>
Ваша группа пользователей не может голосовать
<?php } elseif(!$allowvotepolled) { ?>
Благодарим за Ваш голос
<?php } elseif(!$allowvotethread) { ?>
Голосование было закрыто или истекло время его действия
<?php } ?>
</td>
</tr>
</table>
