...
function renewContent() {
	...
	if(window.confirm('Вы уверены, что хотите восстановить последнее сохранение?')) {
		var data = loadUserdata('home');
		if(in_array((data = trim(data)), ['', 'null', 'false', null, false])) {
			parent.showDialog('Данные не могут быть восстановлены!');
			return;
		}
...
function format(type, para){
	...
	if(!gIsIE){
		switch(type){
			case "Cut":
				sAlert = "Настройки безопасности вашего браузера не позволяют редактору автоматически выполнять операции по вырезанию. Используйте комбинацию клавиш (Ctrl+X), чтобы завершить";
				break;
			case "Copy":
				sAlert = "Настройки безопасности вашего браузера не позволяют редактору автоматически выполнять операции копирования. Используйте комбинацию клавиш (Ctrl+C), чтобы завершить";
				break;
			case "Paste":
				sAlert = "Настройки безопасности вашего браузера не позволяют редактору автоматически выполнять операции вставки. Используйте комбинацию клавиш (Ctrl+V), чтобы завершить";
				break;
		}
	}
	if(sAlert != ""){
...
function doodleBox(event, id) {
	if(parent.$('uchome-ttHtmlEditor') != null) {
		...
		alert("Не могу инициировать граффити");
	}
}
...
function fDisplayElement(element,displayValue) {
	if(gIEVer<=5.01 && gIsIE){
		alert('Поддерживается только IE 5.01 или более поздняя версия!');
		return;
	}
	fHideMenu();
...
function fSetModeTip(obj){
	...
	if(!dvModeTip){
		...
		dv.innerHTML = "Изменить источник";
...
function changeEditType(flag, ev){
	...
	try{
		...
		if(flag){
			...
			ev = ev || event;
			if(ev){
				if(window.confirm("Во время преобразования в обычный текст могут потеряться некоторые элементы форматирования.\nВы уверены, что хотите продолжить?")){
...
function changeEditFull(flag, ev) {
	if(parent.changeEditFull) {
		...
		ele.innerHTML = flag ? 'Экран-' : 'Экран+';
