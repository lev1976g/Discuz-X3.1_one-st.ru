...
function previewFormSubmit() {
	...
	if (!selectedTopicId) {
		alert('Пожалуйста, нажмите на заголовок!');
		return false;
	}
	if (selectedNormalIds.length < 1) {
		alert('Пожалуйста, укажите сообщение!');
		return false;
	}
...
function ajaxGetSearchResultThreads() {
	cloudj('#search_result').html('<tr><td colspan="3">Загрузка...</td></tr>');
...
function addMiniportalList(tid) {
	...
	if (selectedNormalIds.length >= 5) {
		alert('Ваше сообщение достигло 5 статей，вы можите отменить и попробовать снова');
		return false;
	}
...
function removeNormalThreadRecall(displayorder, inNormalEditor) {
	...
	if (threadLi.find('textarea').size()) {
		...
		firstThreadLi.html('<div class="tips">По левому <img src="static/image/admincp/cloud/qun_op_list.png" align="absmiddle" /> 将信息推送到列表</div>');
		firstThreadLi.show();
	}
}
...
function ajaxUploadQQGroupImage() {
	...
	cloudj('#uploadImageResult').text('Изображение загружено...');
