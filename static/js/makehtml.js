	...
	make : function (id, child) {
		...
		x.getJSON(this.url+id+'&_makehtml&r='+(+ new Date()), function(ret){
			...
			if(obj.single) {
				...
				child.innerHTML = '<div class="mk_msg">'+'<a href="'+data['path']+'" target="_blank">'+obj.dom.getAttribute('mktitle')+'</a>Создание завершено'+'</div>';
				if(obj.callback) {
				...
				progress_bar(cent);
				var str = 'Всего нужно создать'+obj.count+'个'+obj.dom.getAttribute('mktitle')+'Файл успешно создан'+obj.makedcount+'个，';
				if(cent != 100) {
