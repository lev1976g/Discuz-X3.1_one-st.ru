...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			...
			showDialog("Количество выбранных файлов превышает установленный предел в \n"+(message === 0 ? " файлов" : "Можно загрузить только " + message + " файлов"), 'notice', null, null, 0, null, null, null, null, sdCloseTime);
			return;
		}
	...
	try {
		...
		if(this.customSettings.uploadSource == 'forum') {
			...
			if(parseInt(data.picid)) {
				...
				newTd = document.createElement("TD");
				newTd.className = 'd';
				newTd.innerHTML = 'Описание <br/><textarea name="title['+data.picid+']" cols="40" rows="2" class="pt"></textarea>';
				newTr.appendChild(newTd);
