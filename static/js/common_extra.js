	...
	if(!obj.id) {
		...
		if(!t) {
			...
			if($(t)) {
				...
				s += '<a href="javascript:;" onclick="$(\'' + inpid + '\').value = \'0\'">Постоянно</a>';
	...
	if(!menu && !pn) {
		...
		if(cover) {
			menu.innerHTML = '<div class="zoominner" id="' + menuid + '_zoomlayer" style="display:none"><p><span class="y"><a id="' + menuid + '_imglink" class="imglink" target="_blank" title="Открыть в новом окне">Открыть в новом окне</a><a id="' + menuid + '_adjust" href="javascipt:;" class="imgadjust" title="Оригинал">Оригинал</a>' +
				'<a href="javascript:;" onclick="hideMenu()" class="imgclose" title="Закрыть">Закрыть</a></span>Увеличить изображение можно колесом мыши</p>' +
	...
	if(typeof zoomgroup == 'object' && zoomgroup[obj.id] && typeof aimgcount == 'object' && aimgcount[zoomgroup[obj.id]]) {
		...
		if(authorlength > 1) {
			...
			if(authorcurrent !== '') {
				...
				picpage += ' <div id="zimg_prev" onmouseover="dragMenuDisabled=true;this.style.backgroundPosition=\'0 50px\'" onmouseout="dragMenuDisabled=false;this.style.backgroundPosition=\'0 -100px\';" onclick="_zoom_page(\'' + paid + '\', ' + (showexif ? 1 : 0) + ')" class="zimg_prev"><strong>Предыдущий</strong></div> ';
				paid = authorcurrent < authorlength - 1 ? authorimgs[authorcurrent + 1] : authorimgs[0];
				picpage += ' <div id="zimg_next" onmouseover="dragMenuDisabled=true;this.style.backgroundPosition=\'100% 50px\'" onmouseout="dragMenuDisabled=false;this.style.backgroundPosition=\'100% -100px\';" onclick="_zoom_page(\'' + paid + '\', ' + (showexif ? 1 : 0) + ')" class="zimg_next"><strong>Следующий</strong></div> ';
			}
			if(picpage) {
...
function _showCreditmenu() {
	if(!$('extcreditmenu_menu')) {
		...
		menu.innerHTML = '<div class="p_opt"><img src="'+ IMGDIR + '/loading.gif" width="16" height="16" class="vm" /> Загрузка...</div>';
		$('append_parent').appendChild(menu);
...
function _setShortcut() {
	$('shortcuttip').onclick = function() {
		var msg = '1. Нажмите на"' + '<a href="javascript:;" class="xi2 xw1" ';
		msg += 'onclick="this.href = \'forum.php?mod=misc&action=shortcut\';this.click();saveUserdata(\'setshortcut\', 1);"';
		msg += '>Создать ярлык на рабочем столе.</a>' + '"После завершения загрузки вы можете сохранить файлы на рабочем столе.<br />';
		msg += '2. Нажмите на"' + '<a href="forum.php?mod=misc&action=shortcut&type=ico" class="xi2 xw1">';
		msg += 'Скачать иконку ICO</a>' + '"После завершения загрузки, щелкните правой кнопкой мыши на ярлыке -> Свойства -> Сменить значок, выберите загруженный значок ICO';
		showDialog(msg, 'notice', 'Добавить ярлык на рабочий стол');
	};
