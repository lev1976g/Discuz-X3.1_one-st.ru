	...
	converthtml : function() {
		...
		if(lastpage) {
			...
			for(var i=1; i<=lastpage; i++) {
				selector += '<option value="'+i+'" '+ (i == curpage ? 'selected' : '') +'>'+i+' стр.</option>';
			}
			selector += '</select>';
			selector += '<span>'+curpage+' стр.</span>';
		}

		$('div.pg').removeClass('pg').addClass('page').html('<a href="'+ prevpagehref +'">Назад</a>'+ selector +'<a href="'+ nextpagehref +'">Вперед</a>');
...
var img = {
	...
	errorhandle : function(obj, is_err_t) {
		...
		if(is_err_t) {
			var parentnode = obj.parent();
			parentnode.find('.loading').remove();
			parentnode.append('<div class="error_text">Обновить</div>');
...
var popup = {
	...
	open : function(pop, type, url) {
		...
		if(typeof pop == 'string') {
			$('#ntcmsg').remove();
			if(type == 'alert') {
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="button2" type="button" value="Ок" onclick="popup.close();"></dd></div>'
			} else if(type == 'confirm') {
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="redirect button2" type="button" value="Ок" href="'+ url +'"><a href="javascript:;" onclick="popup.close();">Отмена</a></dd></div>'
			}
...
var geo = {
	...
	locationerror : function(error) {
		...
		switch(error.code) {
			case error.TIMEOUT:
				geo.errmsg = "Определяю Ваше местоположение, подождите";
				break;
			case error.POSITION_UNAVAILABLE:
				geo.errmsg = 'Не удается обнаружить текущее местоположение';
			    break;
		    case error.PERMISSION_DENIED:
		        geo.errmsg = 'Пожалуйста, позвольте нормальный доступ к вашему текущему местоположению';
		        break;
		    case error.UNKNOWN_ERROR:
		        geo.errmsg = 'Произошла неизвестная ошибка';
		        break;
		}
	},
	...
	init : function() {
		...
		.on('touchmove', function(e) {
			...
			if(!status && $(window).scrollTop() <= 0) {
				...
				contentobj.html('<img src="'+ STATICURL + 'image/mobile/images/icon_arrow.gif" style="vertical-align:middle;margin-right:5px;-moz-transform:rotate(180deg);-webkit-transform:rotate(180deg);-o-transform:rotate(180deg);transform:rotate(180deg);"><span id="refreshtxt">Потяните вниз чтобы обновить</span>');
			...
			if(status == true) {
				...
				if(pullheight >= 0 && pullheight < 150) {
					...
					if(reloadflag) {
						...
						contentobj.find('#refreshtxt').html('Потяните для обновления');
					}
					reloadflag = false;
					...
					if(!reloadflag) {
						...
						contentobj.find('#refreshtxt').html('Отпустите для обновления');
					}
					reloadflag = true;
				}
			}
		...
		.on('touchend', function(e) {
			if(status == true) {
				if(reloadflag) {
					contentobj.html('<img src="'+ STATICURL + 'image/mobile/images/icon_load.gif" style="vertical-align:middle;margin-right:5px;">Загрузка...');
