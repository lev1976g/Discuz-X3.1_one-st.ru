...
function fastpostvalidate(theform, noajaxpost) {
	...
	if(theform.message.value == '' || theform.subject.value == '') {
		s = 'Вы не ввели название или содержание!';
...
function checkForumnew(fid, lasttime) {
	...
	x.get('forum.php?mod=ajax&action=forumchecknew&fid=' + fid + '&time=' + lasttime + '&inajax=yes', function(s){
		if(s > 0) {
			...
			var checknew = {'tid':'', 'thread':{'common':{'className':'', 'val':'<a href="javascript:void(0);" onclick="ajaxget(\'forum.php?mod=ajax&action=forumchecknew&fid=' + fid+ '&time='+lasttime+'&uncheck=1&inajax=yes\', \'forumnew\');">Новое сообщение темы. Нажмите для просмотра.', 'colspan': colspan }}};
