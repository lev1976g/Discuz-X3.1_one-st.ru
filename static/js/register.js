...
var lastusername = '', lastpassword = '', lastemail = '', lastinvitecode = '', stmp = new Array(), modifypwd = false, profileTips = 'Если Вы не хотите изменить пароль, пожалуйста, оставьте пустым';
...
function checkPwdComplexity(firstObj, secondObj, modify) {
	...
	secondObj.onblur = function () {
		if(secondObj.value == '') {
			errormessage(secondObj.id, !modifypwd ? 'Повтор пароля' : profileTips);
		}
		checkpassword(firstObj.id, secondObj.id);
	};
}
...
function checkusername(id) {
	...
	if(unlen < 3 || unlen > 15) {
		errormessage(id, unlen < 3 ? 'Имя пользователя не должно быть меньше трех символов ':' Имя пользователя не более 15 символов');
		return;
	}
	var x = new Ajax();
	$('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
