...
function validate(theform) {
	...
	if(($('postsubmit').name != 'replysubmit' && !($('postsubmit').name == 'editsubmit' && !isfirstpost) && theform.subject.value == "") || !sortid && !special && trim(message) == "") {
		showError('Пожалуйста, укажите название или заполните содержание!');
		return false;
		...
		if(seccodecheck || secqaacheck) {
			...
			if(secqaacheck) {
				...
				if(chkv.indexOf('loading') != -1) {
					...
					showError('Ошибка проверочного кода, попробуйте еще раз');
					chk = 0;
				}
			}
			if(seccodecheck) {
...
function insertAttach(prefix, id) {
	...
	if(extensions != '' && (re.exec(extensions) == null || ext == '')) {
		...
		showError('Данные расширения вложений не поддерживаются!');
		return;
	}
	if(prefix == 'img' && imgexts.indexOf(ext) == -1) {
		reAddAttach(prefix, id);
		showError('Пожалуйста, выберите файл изображения(' + imgexts + ')');
		return;
	}
...
function updateAttach(aid) {
	...
	if(!objupdate.innerHTML) {
		...
		objupdate.innerHTML = '<input type="file" name="attachupdate[paid' + aid + ']"><a href="javascript:;" onclick="updateAttach(' + aid + ')">Отменить</a>';
	} else {
		obj.style.display = '';
		objupdate.innerHTML = '';
	}
