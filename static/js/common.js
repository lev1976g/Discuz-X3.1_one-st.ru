		...
		if(aj.XMLHttpRequest.readyState == 4 && aj.XMLHttpRequest.status == 200) {
			...
			if(aj.recvType == 'HTML') {
				...
				if(!aj.XMLHttpRequest.responseXML || !aj.XMLHttpRequest.responseXML.lastChild || aj.XMLHttpRequest.responseXML.lastChild.localName == 'parsererror') {
					aj.resultHandle('<a href="' + aj.targetUrl + '" target="_blank" style="color:red">Внутренняя ошибка! Содержание невозможно показать!</a>' , aj);
				} else {
		...
		try {
			...
			try {
				...
				try {
					...
					s = 'Внутренняя ошибка! Содержание невозможно показать!';
				}
			}
		}
...
function showWindow(k, url, mode, cache, menuv) {
	...
	var fetchContent = function() {
		...
		if(parseInt(BROWSER.ie) != 6) {
			loadingst = setTimeout(function() {showDialog('', 'info', '<img src="' + IMGDIR + '/loading.gif"> Пожалуйста, подождите...')}, 500);
		}
	};
...
function searchFocus(obj) {
	if(obj.value == 'Пожалуйста, введите искомое') {
		obj.value = '';
	}
...
function toggleBlind(dom) {
	if(dom) {
		if(loadUserdata('is_blindman')) {
			...
			dom.title = 'Закрыть вспомогательный доступа';
		}
	}
}
