	...
	if(!$(menuid)) {
		switch(tag) {
			case 'at':
				...
				str = 'Пожалуйста, введите адрес ссылки:<br /><input type="text" id="atkeyword" style="width:240px" value="" class="px" onkeydown="atFilter(this.value, \'at_list\',\'atListSet\',event);" /><div class="p_pop" id="at_list" style="width:250px;"><ul><li>@ Счет друга, вы можете напомнить ему о целях сообщения</li></ul></div>';
				submitstr = 'seditor_insertunit(\'' + seditorkey + '\', \'@\' + $(\'atkeyword\').value.replace(/<\\/?b>/g, \'\')+\' \'); hideMenu();';
				break;
			case 'url':
				str = 'Введите текстовую ссылку:<br /><input type="text" id="' + ctrlid + '_param_1" sautocomplete="off" style="width: 98%" value="" class="px" />' +
					'<br />Введите текст ссылки:<br /><input type="text" id="' + ctrlid + '_param_2" style="width: 98%" value="" class="px" />';
			...
			case 'img':
				str = 'Пожалуйста, введите адрес:<br /><input type="text" id="' + ctrlid + '_param_1" style="width: 98%" value="" class="px" onchange="loadimgsize(this.value, \'' + seditorkey + '\',\'' + tag + '\')" />' +
					...
					'Ширина (не обязательно): <input type="text" id="' + ctrlid + '_param_3" style="width: 15%" value="" class="px" /></p>';
