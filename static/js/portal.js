...
function strLenCalc(obj, checklen, maxlen) {
	...
	if(curlen <= maxlen) {
		checklen.innerHTML = 'Вступили <b>'+(curlen)+'</b> слов';
		return true;
	} else {
		checklen.innerHTML = 'Превышает <b style="color:red">'+(curlen - maxlen)+'</b> слов';
		return false;
	}
}
...
function check_itemdata_lentgh(form) {
	if(form.title && (!strLenCalc(form.title, "titlechk", form.title.getAttribute('_maxlength')) || !form.title.value)) {
		form.title.focus();
		showDialog('Неверный заголовок', 'error', null, function(){form.title.select();});
		return false;
	}
	if(form.summary && !strLenCalc(form.summary, "summarychk", form.summary.getAttribute('_maxlength'))) {
		form.summary.focus();
		showDialog('Неверная длина', 'error', null, function(){form.summary.select();});
		return false;
	}
	return true;
}