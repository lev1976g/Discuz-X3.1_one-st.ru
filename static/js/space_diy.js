...
drag.extend({
	...
	removeBlock : function (e) {
		...
		if (!confirm('Вы уверены, что хотите удалить? После удаления нельзя будет восстановить данные')) return false;
		...
		if (el != null) {
			...
			x.get('home.php?mod=spacecp&ac=index&op=getblock&blockname='+blockname+'&inajax=1',function(s) {
				if (s) {
					...
					el.id = blockname;
					s = s.replace(/\<script.*\<\/script\>/ig,'<font color="red"> [Сохранить сценарий JS после просмотра] </font>');
					el.innerHTML = s;
...
spaceDiy.extend({
	...
	showEditNvInfo : function () {
		...
		if(!nv) {
			...
			dom.innerHTML = '<span id="editnvinfo" class="edit" style="background-color:#336699;" onclick="spaceDiy.opNvEditInfo();">Создать</span>';
